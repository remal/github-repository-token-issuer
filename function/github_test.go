...
func TestVerifyRequestedScopes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			granted:   &github.InstallationPermissions{Contents: new("write")},
			wantErr:   false,
		},
		{
			name:      "single scope read granted",
			requested: map[string]string{"contents": "read"},
			granted:   &github.InstallationPermissions{Contents: new("read")},
			wantErr:   false,
		},
		{
		...
		},
		{
			...
			granted:   &github.InstallationPermissions{Actions: new("write")},
			wantErr:   false,
		},
		{
			name:      "administration scope",
			requested: map[string]string{"administration": "read"},
			granted:   &github.InstallationPermissions{Administration: new("read")},
			wantErr:   false,
		},
		{
			name:      "attestations scope",
			requested: map[string]string{"attestations": "write"},
			granted:   &github.InstallationPermissions{Attestations: new("write")},
			wantErr:   false,
		},
		{
			name:      "checks scope",
			requested: map[string]string{"checks": "write"},
			granted:   &github.InstallationPermissions{Checks: new("write")},
			wantErr:   false,
		},
		{
			name:      "dependabot_secrets scope",
			requested: map[string]string{"dependabot_secrets": "read"},
			granted:   &github.InstallationPermissions{DependabotSecrets: new("read")},
			wantErr:   false,
		},
		{
			name:      "deployments scope",
			requested: map[string]string{"deployments": "write"},
			granted:   &github.InstallationPermissions{Deployments: new("write")},
			wantErr:   false,
		},
		{
			name:      "discussions scope",
			requested: map[string]string{"discussions": "read"},
			granted:   &github.InstallationPermissions{Discussions: new("read")},
			wantErr:   false,
		},
		{
			name:      "environments scope",
			requested: map[string]string{"environments": "write"},
			granted:   &github.InstallationPermissions{Environments: new("write")},
			wantErr:   false,
		},
		{
			name:      "issues scope",
			requested: map[string]string{"issues": "write"},
			granted:   &github.InstallationPermissions{Issues: new("write")},
			wantErr:   false,
		},
		{
			name:      "merge_queues scope",
			requested: map[string]string{"merge_queues": "read"},
			granted:   &github.InstallationPermissions{MergeQueues: new("read")},
			wantErr:   false,
		},
		{
			name:      "packages scope",
			requested: map[string]string{"packages": "write"},
			granted:   &github.InstallationPermissions{Packages: new("write")},
			wantErr:   false,
		},
		{
			name:      "pages scope",
			requested: map[string]string{"pages": "read"},
			granted:   &github.InstallationPermissions{Pages: new("read")},
			wantErr:   false,
		},
		{
			name:      "projects scope maps to RepositoryProjects",
			requested: map[string]string{"projects": "write"},
			granted:   &github.InstallationPermissions{RepositoryProjects: new("write")},
			wantErr:   false,
		},
		{
			name:      "pull_requests scope",
			requested: map[string]string{"pull_requests": "write"},
			granted:   &github.InstallationPermissions{PullRequests: new("write")},
			wantErr:   false,
		},
		{
			name:      "secret_scanning scope maps to SecretScanningAlerts",
			requested: map[string]string{"secret_scanning": "read"},
			granted:   &github.InstallationPermissions{SecretScanningAlerts: new("read")},
			wantErr:   false,
		},
		{
			name:      "secrets scope",
			requested: map[string]string{"secrets": "write"},
			granted:   &github.InstallationPermissions{Secrets: new("write")},
			wantErr:   false,
		},
		{
			name:      "statuses scope",
			requested: map[string]string{"statuses": "read"},
			granted:   &github.InstallationPermissions{Statuses: new("read")},
			wantErr:   false,
		},
		{
			name:      "workflows scope",
			requested: map[string]string{"workflows": "write"},
			granted:   &github.InstallationPermissions{Workflows: new("write")},
			wantErr:   false,
		},
		{
		...
		},
		{
			...
			granted: &github.InstallationPermissions{
				...
				Attestations:         new("write"),
				Checks:               new("write"),
				Contents:             new("write"),
				DependabotSecrets:    new("write"),
				Deployments:          new("write"),
				Discussions:          new("write"),
				Environments:         new("write"),
				Issues:               new("write"),
				MergeQueues:          new("write"),
				Packages:             new("write"),
				Pages:                new("write"),
				RepositoryProjects:   new("write"),
				PullRequests:         new("write"),
...
func TestGetInstallationID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockResponse: &github.Installation{ID: new(int64(12345))},
		...
		},
		{
			...
			mockResponse: &github.Installation{ID: new(int64(99999))},
			mockResp:     &github.Response{Response: &http.Response{StatusCode: http.StatusOK}},
			mockErr:      nil,
			wantID:       99999,
			wantErr:      false,
...
func TestGetInstallationID_RetryOnNetworkError(t *testing.T) {
	...
	t.Cleanup(func() { retryBackoffBase = 30 * time.Second })
	ctx := context.Background()
	callCount := 0
	...
	mock := &mockAppsService{
		findRepoInstallation: func(ctx context.Context, owner, repo string) (*github.Installation, *github.Response, error) {
			...
			return &github.Installation{ID: new(int64(12345))},
...
func TestCreateInstallationToken(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockToken: &github.InstallationToken{
				Token:       new("ghs_test123"),
				ExpiresAt:   &github.Timestamp{Time: testTime},
				Permissions: &github.InstallationPermissions{Contents: new("write")},
			},
